...
class M_tpekerjaan extends CI_Model{
	 ...
	 * 
	 * Untuk mengubah seluruh data yang db.tpekerjaan.VALIDTO = null
	 * 
	 * @param array $data
	 * @return json
	 */
	function validtoall_update($data){
		...
		if($this->db->get_where('tpekerjaan', $where)->num_rows() > 0){
			/*
			 * Data Exist
			 */
			...
			$this->db->where($where)->update('tpekerjaan', $arrdatau);
			
			$result = $this->db->get('tpekerjaan')->result();
			$total  = $this->db->get('tpekerjaan')->num_rows();
			...
			$last   = $this->db->get('tpekerjaan')->row();
			$total  = $this->db->get('tpekerjaan')->num_rows();
