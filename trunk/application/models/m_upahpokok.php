...
class M_upahpokok extends CI_Model{
	 ...
	 * 
	 * Untuk mengubah seluruh data yang db.upahpokok.VALIDTO = null
	 * 
	 * @param array $data
	 * @return json
	 */
	function validtoall_update($data){
		...
		if($this->db->get_where('upahpokok', $where)->num_rows() > 0){
			/*
			 * Data Exist
			 */
			...
			$this->db->where($where)->update('upahpokok', $arrdatau);
			
			$result = $this->db->get('upahpokok')->result();
			$total  = $this->db->get('upahpokok')->num_rows();
			...
			$last   = $this->db->get('upahpokok')->row();
			$total  = $this->db->get('upahpokok')->num_rows();
