...
class M_hitungpresensi extends CI_Model{
	...
	function JamKerja($bulangaji){
		...
		* ----------------------- Ini adalah query ujicoba potong JAMLEMBUR saat MAKAN SIANG dan MAKAN MALAM ---------------------------------
		...
		SELECT DATE_FORMAT(t1.TJMASUK,'%Y-%m-%d') as TANGGAL, t1.NIK, t1.TJMASUK, t1.TJKELUAR,t4.TJMASUK as TJLEMBUR, sum(TIMESTAMPDIFF(MINUTE,t1.TJMASUK,t4.TJMASUK)) as JAMKERJA, SUM(TIMESTAMPDIFF(MINUTE,t4.TJMASUK,t1.TJKELUAR)) as LEMBURKOTOR,
		IF((
		IF(TIME(t1.TJKELUAR) < TIME('13:00:00'),IF(TIME(t1.TJKELUAR) > TIME('12:00:00'),TIME_FORMAT(TIME(t1.TJKELUAR),'%i'),0),IF(TIME(t1.TJKELUAR)> TIME('13:00:00'),60,0)))=0,
		IF(TIME(t1.TJKELUAR) < TIME('18:30:00'),IF(TIME(t1.TJKELUAR) > TIME('18:00:00'),TIME_FORMAT(TIME(t1.TJKELUAR),'%i'),0),IF(TIME(t1.TJKELUAR)> TIME('18:30:00'),60,0)),
		IF(TIME(t1.TJKELUAR) < TIME('13:00:00'),IF(TIME(t1.TJKELUAR) > TIME('12:00:00'),TIME_FORMAT(TIME(t1.TJKELUAR),'%i'),0),IF(TIME(t1.TJKELUAR)> TIME('13:00:00'),60,0))) AS POTONGMAKAN,
		...
		SELECT IF((
		IF(TIME('2012-08-08 20:39:00') < TIME('13:00:00'),IF(TIME('2012-08-08 20:39:00') > TIME('12:00:00'),TIME_FORMAT(TIME('2012-08-08 20:39:00'),'%i'),0),IF(TIME('2012-08-08 20:39:00')> TIME('13:00:00'),60,0)))=0,
		IF(TIME('2012-08-08 20:39:00') < TIME('19:00:00'),IF(TIME('2012-08-08 20:39:00') > TIME('18:00:00'),TIME_FORMAT(TIME('2012-08-08 20:39:00'),'%i'),0),IF(TIME('2012-08-08 20:39:00')> TIME('19:00:00'),60,0)),
		IF(TIME('2012-08-08 20:39:00') < TIME('13:00:00'),IF(TIME('2012-08-08 20:39:00') > TIME('12:00:00'),TIME_FORMAT(TIME('2012-08-08 20:39:00'),'%i'),0),IF(TIME('2012-08-08 20:39:00')> TIME('13:00:00'),60,0))) as BERSIH;
