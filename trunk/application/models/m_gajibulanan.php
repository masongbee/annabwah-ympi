...
class M_gajibulanan extends CI_Model{
	...
	function update_detilgaji_rptlembur($bulan, $tglmulai, $tglsampai){
		/**
		 ...
		 */
		//??????????????? HITUNG RPTLEMBUR UNTUK HITUNG PRESENSI TIDAK UNTUK YANG KHUSUS, karena di db.detilgaji tidak ada UPENGALI
		// $sql = "UPDATE detilgaji AS t1
		// 	JOIN (
		...
		// 			SELECT t111.NIK, ((t111.SATLEMBUR *
		// 				(IF(t111.UPENGALI = 'A', t112.RPUPAHPOKOK,
		// 					IF(t111.UPENGALI = 'B', (t112.RPUPAHPOKOK + t112.RPTTRANSPORT + t112.RPTPEKERJAAN + t112.RPTSHIFT),
		// 						IF(t111.UPENGALI = 'C', (t112.RPUPAHPOKOK + t112.RPUMSK + t112.RPTJABATAN + t112.RPTBHS + t112.RPTANAK + t112.RPTISTRI),
		// 							(t112.RPUPAHPOKOK + t112.RPUMSK + t112.RPTJABATAN + t112.RPTANAK + t112.RPTISTRI)))))) / 173) AS RPTLEMBUR
		// 			FROM (
		// 				SELECT NIK, SATLEMBUR, UPENGALI
		// 				FROM hitungpresensi
		// 				WHERE CAST(t114.BULAN AS UNSIGNED) = CAST('".$bulan."' AS UNSIGNED)) AS t111
		// 			JOIN (
	...
	function update_detilgaji_rptqcp_bynik($bulan, $tglmulai, $tglsampai){
		$sql = "UPDATE detilgaji AS t1
			...
			SET t1.RPTQCP = (t2.RPQCP * t1.HARIKERJA)";
		$this->db->query($sql);
	}
	...
	function update_detilgaji_rpthadir_bynik($bulan, $tglmulai, $tglsampai){
		$sql = "UPDATE detilgaji AS t1
			JOIN (
				SELECT NIK, FPENGALI, RPTHADIR
				FROM tkehadiran
				WHERE CAST(BULAN AS UNSIGNED) = CAST('".$bulan."' AS UNSIGNED)
			) AS t2 ON(CAST(t1.BULAN AS UNSIGNED) = CAST('".$bulan."' AS UNSIGNED)
				AND t2.NIK = t1.NIK)
			SET t1.RPTHADIR = (t2.RPTHADIR * (IF(t2.FPENGALI = 'H', t1.HARIKERJA, 1)))";
		$this->db->query($sql);
	}
	...
	function update_detilgaji_rppjamsostek_bygrade($bulan){
		$sql = "UPDATE detilgaji AS t1
			JOIN (
				...
				AND t2.GRADE = t1.GRADE)
			SET t1.RPPJAMSOSTEK = ((t2.PERSENTASE / 100) * (t1.RPUPAHPOKOK + t1.RPUMSK + t1.RPTJABATAN + t1.RPTBHS + t1.RPTANAK + t1.RPTISTRI))";
		$this->db->query($sql);
	}
	...
	function hitunggaji_all($bulan, $tglmulai, $tglsampai){
		...
		$sqlu_harikerja = "UPDATE detilgaji AS t1
			...
			SET t1.HARIKERJA = t2.TOTAL_HARIKERJA";
		$this->db->query($sqlu_harikerja);
		//update db.detilgaji.HARIKERJA dari db.presensibln (karyawan khusus)
		$sqlu_harikerja_khusus = "UPDATE detilgaji AS t1
			JOIN (
				SELECT presensibln.NIK, presensibln.HARIKERJA AS TOTAL_HARIKERJA
				FROM presensibln
				WHERE presensibln.BULAN = '".$bulan."'
			) AS t2 ON(t2.NIK = t1.NIK)
			SET t1.HARIKERJA = t2.TOTAL_HARIKERJA";
		$this->db->query($sqlu_harikerja_khusus);

		//update db.detilgaji.EXTRADAY dari db.hitungpresensi
		$sqlu_extraday = "UPDATE detilgaji AS t1
			JOIN (
				SELECT hitungpresensi.NIK, SUM(hitungpresensi.EXTRADAY) AS TOTAL_EXTRADAY
				FROM hitungpresensi
				WHERE hitungpresensi.EXTRADAY = 1
					AND hitungpresensi.TANGGAL >= STR_TO_DATE('".$tglmulai."', '%Y-%m-%d')
					AND hitungpresensi.TANGGAL <= STR_TO_DATE('".$tglsampai."', '%Y-%m-%d')
				GROUP BY hitungpresensi.NIK
			) AS t2 ON(t2.NIK = t1.NIK)
			SET t1.EXTRADAY = t2.TOTAL_EXTRADAY";
		$this->db->query($sqlu_extraday);
		//update db.detilgaji.EXTRADAY dari db.presensibln (karyawan khusus)
		$sqlu_extraday_khusus = "UPDATE detilgaji AS t1
			JOIN (
				SELECT presensibln.NIK, presensibln.EXTRADAY AS TOTAL_EXTRADAY
				FROM presensibln
				WHERE presensibln.BULAN = '".$bulan."'
			) AS t2 ON(t2.NIK = t1.NIK)
			SET t1.EXTRADAY = t2.TOTAL_EXTRADAY";
		$this->db->query($sqlu_extraday_khusus);

		//update db.detilgaji.XPOTONG dari db.hitungpresensi
		$sqlu_xpotong_jmljamkurang = "UPDATE detilgaji AS t1
			JOIN (
				SELECT hitungpresensi.NIK, SUM(hitungpresensi.XPOTONG) AS TOTAL_XPOTONG, 
					SUM(IFNULL(TERLAMBAT, 0) + IFNULL(PLGLBHAWAL, 0) + IFNULL(IZINPRIBADI, 0)) AS TOTAL_JMLJAMKURANG
				FROM hitungpresensi
				WHERE hitungpresensi.XPOTONG = 1
					AND hitungpresensi.TANGGAL >= STR_TO_DATE('".$tglmulai."', '%Y-%m-%d')
					AND hitungpresensi.TANGGAL <= STR_TO_DATE('".$tglsampai."', '%Y-%m-%d')
				GROUP BY hitungpresensi.NIK
			) AS t2 ON(t2.NIK = t1.NIK)
			SET t1.XPOTONG = t2.TOTAL_XPOTONG,
				t1.JMLJAMKURANG = t2.TOTAL_JMLJAMKURANG";
		$this->db->query($sqlu_xpotong_jmljamkurang);
		//update db.detilgaji.XPOTONG dari db.presensibln (karyawan khusus)
		$sqlu_xpotong_jmljamkurang_khusus = "UPDATE detilgaji AS t1
			JOIN (
				SELECT presensibln.NIK, presensibln.XPOTONG AS TOTAL_XPOTONG, presensibln.JMLJAMKURANG AS TOTAL_JMLJAMKURANG
				FROM presensibln
				WHERE presensibln.BULAN = '".$bulan."'
			) AS t2 ON(t2.NIK = t1.NIK)
			SET t1.XPOTONG = t2.TOTAL_XPOTONG,
				t1.JMLJAMKURANG = t2.TOTAL_JMLJAMKURANG";
		$this->db->query($sqlu_xpotong_jmljamkurang_khusus);

		//update db.detilgaji.SATLEMBUR dari db.hitungpresensi
		$sqlu_satlembur = "UPDATE detilgaji AS t1
			JOIN (
				SELECT hitungpresensi.NIK, SUM(hitungpresensi.SATLEMBUR) AS TOTAL_SATLEMBUR
				FROM hitungpresensi
				WHERE hitungpresensi.SATLEMBUR = 1
					AND hitungpresensi.TANGGAL >= STR_TO_DATE('".$tglmulai."', '%Y-%m-%d')
					AND hitungpresensi.TANGGAL <= STR_TO_DATE('".$tglsampai."', '%Y-%m-%d')
				GROUP BY hitungpresensi.NIK
			) AS t2 ON(t2.NIK = t1.NIK)
			SET t1.SATLEMBUR = t2.TOTAL_SATLEMBUR";
		$this->db->query($sqlu_satlembur);
		//update db.detilgaji.SATLEMBUR dari db.presensibln (karyawan khusus)
		$sqlu_satlembur_khusus = "UPDATE detilgaji AS t1
			JOIN (
				SELECT presensibln.NIK, presensibln.SATLEMBUR AS TOTAL_SATLEMBUR
				FROM presensibln
				WHERE presensibln.BULAN = '".$bulan."'
			) AS t2 ON(t2.NIK = t1.NIK)
			SET t1.SATLEMBUR = t2.TOTAL_SATLEMBUR";
		...
		// }
		$this->update_detilgaji_rptlembur($bulan, $tglmulai, $tglsampai);
		...
		if(sizeof($records_rpthadir) > 0){
			...
			$this->update_detilgaji_rpthadir_bynik($bulan, $tglmulai, $tglsampai);
		}
		...
		/* 27.a. */
		$sql_rppjamsostek = "SELECT NIK, GRADE, KODEJAB
			FROM pjamsostek
			WHERE CAST(DATE_FORMAT(VALIDFROM,'%Y%m') AS UNSIGNED) <= CAST('".$bulan."' AS UNSIGNED)
				AND (VALIDTO IS NULL OR CAST(DATE_FORMAT(VALIDTO,'%Y%m') AS UNSIGNED) >= CAST('".$bulan."' AS UNSIGNED))
				AND CAST(BULANMULAI AS UNSIGNED) <= CAST('".$bulan."' AS UNSIGNED)
				AND CAST(BULANSAMPAI AS UNSIGNED) >= CAST('".$bulan."' AS UNSIGNED)
			LIMIT 1";
		$records_rppjamsostek = $this->db->query($sql_rppjamsostek)->result();
		
		/* 27.b. */
		if(sizeof($records_rppjamsostek) > 0){
			/* urutan upah pokok ke-1 berdasarkan GRADE */
			$this->update_detilgaji_rppjamsostek_bygrade($bulan);
			/* urutan upah pokok ke-2 berdasarkan KODEJAB */
			$this->update_detilgaji_rppjamsostek_bykodejab($bulan);
			/* urutan upah pokok ke-3 berdasarkan GRADE+KODEJAB */
			$this->update_detilgaji_rppjamsostek_bygradekodejab($bulan);
			/* urutan upah pokok ke-4 berdasarkan NIK */
