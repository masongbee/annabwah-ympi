...
class C_rptpresensi extends CI_Controller {
	
	function __construct(){
		parent::__construct();		
		$this->load->model('m_rptpresensi', '', TRUE);
	}
	...
	function getAll(){
		...
		/* Collect Filter */
		$groups 	= ($this->input->post('group', TRUE) ? $this->input->post('group', TRUE) : '');
		$sorts   = ($this->input->post('sort', TRUE) ? $this->input->post('sort', TRUE) : null);
		 ...
		 * Processing Data
		 */
		$result = $this->m_rptpresensi->getAllData($groups,$sorts,$filters,$start, $page, $limit);
