...
class C_tambahan extends CI_Controller {
	...
	function __construct(){
		parent::__construct();		
		$this->load->model('m_tambahan', '', TRUE);
	}
	...
	function getAll(){
		 ...
		 * Processing Data
		 */
		$result = $this->m_tambahan->getAll($start, $page, $limit);
		echo json_encode($result);
	}
	
	function save(){
		/*
		 * Collect Data ==> diambil dari [model.tambahan]
		 */
		 ...
		 * Processing Data
		 */
		$result = $this->m_tambahan->save($data);
		echo json_encode($result);
	}
	...
	function delete(){
		 ...
		 * Processing Data
		 */
		$result = $this->m_tambahan->delete($data);
		echo json_encode($result);
	}
	...
	function export2Excel(){
		...
		$filename='tambahan.xlsx'; //save our workbook as this file name
	...
	function export2PDF(){
		...
		$this->html2pdf->filename('tambahan.pdf');
		
		//Set the paper defaults
		$this->html2pdf->paper('a4', 'portrait');
		
		//Load html view
		$this->html2pdf->html($this->load->view('pdf_tambahan', $data, true));
	...
	function printRecords(){
		...
		$data["records"] = $getdata;
		$data["table"] = "tambahan";
		$print_view=$this->load->view("p_tambahan.php",$data,TRUE);
		if(!file_exists("temp")){
			mkdir("temp");
		}
		$print_file=fopen("temp/tambahan.html","w+");
