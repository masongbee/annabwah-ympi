...
class C_uangsimpati extends CI_Controller {
	
	function __construct(){
		parent::__construct();		
		$this->load->model('m_uangsimpati', '', TRUE);
	}
	...
	function getAll(){
		 ...
		 * Processing Data
		 */
		$result = $this->m_uangsimpati->getAll($start, $page, $limit);
		echo json_encode($result);
	}
	
	function save(){
		/*
		 * Collect Data ==> diambil dari [model.uangsimpati]
		 */
		 ...
		 * Processing Data
		 */
		$result = $this->m_uangsimpati->save($data);
		echo json_encode($result);
	}
	...
	function delete(){
		 ...
		 * Processing Data
		 */
		$result = $this->m_uangsimpati->delete($data);
		echo json_encode($result);
	}
	...
	function export2Excel(){
		...
		$filename='uangsimpati.xlsx'; //save our workbook as this file name
	...
	function export2PDF(){
		...
		$this->html2pdf->filename('uangsimpati.pdf');
		
		//Set the paper defaults
		$this->html2pdf->paper('a4', 'portrait');
		
		//Load html view
		$this->html2pdf->html($this->load->view('pdf_uangsimpati', $data, true));
	...
	function printRecords(){
		...
		$data["records"] = $getdata;
		$data["table"] = "uangsimpati";
		$print_view=$this->load->view("p_uangsimpati.php",$data,TRUE);
		if(!file_exists("temp")){
			mkdir("temp");
		}
		$print_file=fopen("temp/uangsimpati.html","w+");
