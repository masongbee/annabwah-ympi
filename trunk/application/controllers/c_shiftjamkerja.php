...
class C_shiftjamkerja extends CI_Controller {
	
	function __construct(){
		parent::__construct();		
		$this->load->model('m_shiftjamkerja', '', TRUE);
	}
	...
	function getAll(){
		...
		$namashift 	= ($this->input->post('NAMASHIFT', TRUE) ? $this->input->post('NAMASHIFT', TRUE) : '');
		$shiftke 	= ($this->input->post('SHIFTKE', TRUE) ? $this->input->post('SHIFTKE', TRUE) : '');
		
		/*
		 * Processing Data
		 */
		$result = $this->m_shiftjamkerja->getAll($namashift,$shiftke,$start, $page, $limit);
		echo json_encode($result);
	}
	
	function save(){
		/*
		 * Collect Data ==> diambil dari [model.shiftjamkerja]
		 */
		 ...
		 * Processing Data
		 */
		$result = $this->m_shiftjamkerja->save($data);
		echo json_encode($result);
	}
	...
	function delete(){
		 ...
		 * Processing Data
		 */
		$result = $this->m_shiftjamkerja->delete($data);
		echo json_encode($result);
	}
	...
	function export2Excel(){
		...
		foreach($data as $record)
		{
			...
			foreach ($data[0] as $key => $value)
			{
				...
				if($key == strtoupper('shiftjamkerja')){
		...
		$filename='shiftjamkerja.xlsx'; //save our workbook as this file name
	...
	function export2PDF(){
		...
		$this->html2pdf->filename('shiftjamkerja.pdf');
		
		//Set the paper defaults
		$this->html2pdf->paper('a4', 'portrait');
		
		//Load html view
		$this->html2pdf->html($this->load->view('pdf_shiftjamkerja', $data, true));
	...
	function printRecords(){
		...
		$data["records"] = $getdata;
		$data["table"] = "shiftjamkerja";
		$print_view=$this->load->view("p_shiftjamkerja.php",$data,TRUE);
		if(!file_exists("temp")){
			mkdir("temp");
		}
		$print_file=fopen("temp/shiftjamkerja.html","w+");
