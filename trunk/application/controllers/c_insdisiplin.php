...
class C_insdisiplin extends CI_Controller {
	
	function __construct(){
		parent::__construct();		
		$this->load->model('m_insdisiplin', '', TRUE);
	}
	...
	function getAll(){
		 ...
		 * Processing Data
		 */
		$result = $this->m_insdisiplin->getAll($start, $page, $limit);
		echo json_encode($result);
	}
	
	function save(){
		/*
		 * Collect Data ==> diambil dari [model.insdisiplin]
		 */
		 ...
		 * Processing Data
		 */
		$result = $this->m_insdisiplin->save($data);
		echo json_encode($result);
	}
	...
	function delete(){
		 ...
		 * Processing Data
		 */
		$result = $this->m_insdisiplin->delete($data);
		echo json_encode($result);
	}
	...
	function export2Excel(){
		...
		$filename='insdisiplin.xlsx'; //save our workbook as this file name
	...
	function export2PDF(){
		...
		$this->html2pdf->filename('insdisiplin.pdf');
		
		//Set the paper defaults
		$this->html2pdf->paper('a4', 'portrait');
		
		//Load html view
		$this->html2pdf->html($this->load->view('pdf_insdisiplin', $data, true));
	...
	function printRecords(){
		...
		$data["records"] = $getdata;
		$data["table"] = "insdisiplin";
		$print_view=$this->load->view("p_insdisiplin.php",$data,TRUE);
		if(!file_exists("temp")){
			mkdir("temp");
		}
