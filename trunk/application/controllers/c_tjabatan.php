...
class C_tjabatan extends CI_Controller {
	
	function __construct(){
		parent::__construct();		
		$this->load->model('m_tjabatan', '', TRUE);
	}
	...
	function getAll(){
		 ...
		 * Processing Data
		 */
		$result = $this->m_tjabatan->getAll($start, $page, $limit);
		echo json_encode($result);
	}
	
	function save(){
		/*
		 * Collect Data ==> diambil dari [model.tjabatan]
		 */
		 ...
		 * Processing Data
		 */
		$result = $this->m_tjabatan->save($data);
		echo json_encode($result);
	}
	...
	function delete(){
		 ...
		 * Processing Data
		 */
		$result = $this->m_tjabatan->delete($data);
		echo json_encode($result);
	}
	...
	function export2Excel(){
		...
		$filename='tjabatan.xlsx'; //save our workbook as this file name
	...
	function export2PDF(){
		...
		$this->html2pdf->filename('tjabatan.pdf');
		
		//Set the paper defaults
		$this->html2pdf->paper('a4', 'portrait');
		
		//Load html view
		$this->html2pdf->html($this->load->view('pdf_tjabatan', $data, true));
	...
	function printRecords(){
		...
		$data["records"] = $getdata;
		$data["table"] = "tjabatan";
		$print_view=$this->load->view("p_tjabatan.php",$data,TRUE);
		if(!file_exists("temp")){
			mkdir("temp");
		}
		$print_file=fopen("temp/tjabatan.html","w+");
