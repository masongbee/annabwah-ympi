...
class C_riwayattraining extends CI_Controller {
	...
	function getAll(){
		 ...
		 * Processing Data
		 */
		$result = $this->m_riwayattraining->getAll($nik, $start, $page, $limit);
	...
	function export2Excel(){
		...
		foreach($data as $record)
		{
			...
			foreach ($data[0] as $key => $value)
			{
				...
				if($key == strtoupper('riwayattraining')){
		...
		$filename='riwayattraining.xlsx'; //save our workbook as this file name
	...
	function export2PDF(){
		...
		$this->html2pdf->filename('riwayattraining.pdf');
	...
	function printRecords(){
		...
		$data["records"] = $getdata;
		$data["table"] = "riwayattraining";
		$print_view=$this->load->view("p_riwayattraining.php",$data,TRUE);
		if(!file_exists("temp")){
			mkdir("temp");
		}
		$print_file=fopen("temp/riwayattraining.html","w+");
