...
class C_tkeluarga extends CI_Controller {
	
	function __construct(){
		parent::__construct();		
		$this->load->model('m_tkeluarga', '', TRUE);
	}
	...
	function getAll(){
		 ...
		 * Processing Data
		 */
		$result = $this->m_tkeluarga->getAll($start, $page, $limit);
		echo json_encode($result);
	}
	
	function save(){
		/*
		 * Collect Data ==> diambil dari [model.tkeluarga]
		 */
		 ...
		 * Processing Data
		 */
		$result = $this->m_tkeluarga->save($data);
		echo json_encode($result);
	}
	...
	function delete(){
		 ...
		 * Processing Data
		 */
		$result = $this->m_tkeluarga->delete($data);
		echo json_encode($result);
	}
	...
	function export2Excel(){
		...
		$filename='tkeluarga.xlsx'; //save our workbook as this file name
	...
	function export2PDF(){
		...
		$this->html2pdf->filename('tkeluarga.pdf');
		
		//Set the paper defaults
		$this->html2pdf->paper('a4', 'portrait');
		
		//Load html view
		$this->html2pdf->html($this->load->view('pdf_tkeluarga', $data, true));
	...
	function printRecords(){
		...
		$data["records"] = $getdata;
		$data["table"] = "tkeluarga";
		$print_view=$this->load->view("p_tkeluarga.php",$data,TRUE);
		if(!file_exists("temp")){
			mkdir("temp");
		}
		$print_file=fopen("temp/tkeluarga.html","w+");
	...
	function validtoall_update(){
		 ...
		 * Processing Data
		 */
		$result = $this->m_tkeluarga->validtoall_update($data);
		echo json_encode($result);
	}
}