...
class C_potongansp extends CI_Controller {
	
	function __construct(){
		parent::__construct();		
		$this->load->model('m_potongansp', '', TRUE);
	}
	...
	function getAll(){
		 ...
		 * Processing Data
		 */
		$result = $this->m_potongansp->getAll($start, $page, $limit);
		echo json_encode($result);
	}
	
	function save(){
		/*
		 * Collect Data ==> diambil dari [model.potongansp]
		 */
		 ...
		 * Processing Data
		 */
		$result = $this->m_potongansp->save($data);
		echo json_encode($result);
	}
	...
	function delete(){
		 ...
		 * Processing Data
		 */
		$result = $this->m_potongansp->delete($data);
		echo json_encode($result);
	}
	...
	function export2Excel(){
		...
		$filename='potongansp.xlsx'; //save our workbook as this file name
	...
	function export2PDF(){
		...
		$this->html2pdf->filename('potongansp.pdf');
		
		//Set the paper defaults
		$this->html2pdf->paper('a4', 'portrait');
		
		//Load html view
		$this->html2pdf->html($this->load->view('pdf_potongansp', $data, true));
	...
	function printRecords(){
		...
		$data["records"] = $getdata;
		$data["table"] = "potongansp";
		$print_view=$this->load->view("p_potongansp.php",$data,TRUE);
		if(!file_exists("temp")){
			mkdir("temp");
		}
		$print_file=fopen("temp/potongansp.html","w+");
