...
class C_permohonancuti extends CI_Controller {
	...
	function save(){
		/*
		 * Collect Data ==> diambil dari [model.permohonancuti]
		 */
		 ...
		 * Processing Data
		 */
		$result = $this->m_permohonancuti->save($data);
		echo json_encode($result);
	}
	...
	function delete(){
		 ...
		 * Processing Data
		 */
		$result = $this->m_permohonancuti->delete($data);
		echo json_encode($result);
	}
	...
	function export2Excel(){
		...
		foreach($data as $record)
		{
			...
			foreach ($data[0] as $key => $value)
			{
				...
				if($key == strtoupper('permohonancuti')){
		...
		$filename='permohonancuti.xlsx'; //save our workbook as this file name
	...
	function export2PDF(){
		...
		$this->html2pdf->filename('permohonancuti.pdf');
		
		//Set the paper defaults
		$this->html2pdf->paper('a4', 'portrait');
		
		//Load html view
		$this->html2pdf->html($this->load->view('pdf_permohonancuti', $data, true));
	...
	function printRecords(){
		...
		$data["records"] = $getdata;
		$data["table"] = "permohonancuti";
		$print_view=$this->load->view("p_permohonancuti.php",$data,TRUE);
		if(!file_exists("temp")){
			mkdir("temp");
		}
		$print_file=fopen("temp/permohonancuti.html","w+");
