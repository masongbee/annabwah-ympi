...
class C_tshift extends CI_Controller {
	
	function __construct(){
		parent::__construct();		
		$this->load->model('m_tshift', '', TRUE);
	}
	...
	function getAll(){
		 ...
		 * Processing Data
		 */
		$result = $this->m_tshift->getAll($start, $page, $limit);
		echo json_encode($result);
	}
	
	function save(){
		/*
		 * Collect Data ==> diambil dari [model.tshift]
		 */
		 ...
		 * Processing Data
		 */
		$result = $this->m_tshift->save($data);
		echo json_encode($result);
	}
	...
	function delete(){
		 ...
		 * Processing Data
		 */
		$result = $this->m_tshift->delete($data);
		echo json_encode($result);
	}
	...
	function export2Excel(){
		...
		$filename='tshift.xlsx'; //save our workbook as this file name
	...
	function export2PDF(){
		...
		$this->html2pdf->filename('tshift.pdf');
		
		//Set the paper defaults
		$this->html2pdf->paper('a4', 'portrait');
		
		//Load html view
		$this->html2pdf->html($this->load->view('pdf_tshift', $data, true));
	...
	function printRecords(){
		...
		$data["records"] = $getdata;
		$data["table"] = "tshift";
		$print_view=$this->load->view("p_tshift.php",$data,TRUE);
		if(!file_exists("temp")){
			mkdir("temp");
		}
		$print_file=fopen("temp/tshift.html","w+");
	...
	function validtoall_update(){
		 ...
		 * Processing Data
		 */
		$result = $this->m_tshift->validtoall_update($data);
		echo json_encode($result);
	}
}