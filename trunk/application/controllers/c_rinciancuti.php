...
class C_rinciancuti extends CI_Controller {
	...
	function getAll(){
		...
		$nocuti  =   ($this->input->post('NOCUTI', TRUE) ? $this->input->post('NOCUTI', TRUE) : NULL);
	...
	function save(){
		/*
		 * Collect Data ==> diambil dari [model.rinciancuti]
		 */
	...
	function export2Excel(){
		...
		$filename='rinciancuti.xlsx'; //save our workbook as this file name
	...
	function export2PDF(){
		...
		$this->html2pdf->filename('rinciancuti.pdf');
		
		//Set the paper defaults
		$this->html2pdf->paper('a4', 'portrait');
		
		//Load html view
		$this->html2pdf->html($this->load->view('pdf_rinciancuti', $data, true));
	...
	function printRecords(){
		...
		$data["records"] = $getdata;
		$data["table"] = "rinciancuti";
		$print_view=$this->load->view("p_rinciancuti.php",$data,TRUE);
		if(!file_exists("temp")){
			mkdir("temp");
		}
		$print_file=fopen("temp/rinciancuti.html","w+");
