...
class C_potongan extends CI_Controller {
	...
	function __construct(){
		parent::__construct();		
		$this->load->model('m_potongan', '', TRUE);
	}
	...
	function getAll(){
		 ...
		 * Processing Data
		 */
		$result = $this->m_potongan->getAll($start, $page, $limit);
		echo json_encode($result);
	}
	
	function save(){
		/*
		 * Collect Data ==> diambil dari [model.potongan]
		 */
		 ...
		 * Processing Data
		 */
		$result = $this->m_potongan->save($data);
		echo json_encode($result);
	}
	...
	function delete(){
		 ...
		 * Processing Data
		 */
		$result = $this->m_potongan->delete($data);
		echo json_encode($result);
	}
	...
	function export2Excel(){
		...
		$filename='potongan.xlsx'; //save our workbook as this file name
	...
	function export2PDF(){
		...
		$this->html2pdf->filename('potongan.pdf');
		
		//Set the paper defaults
		$this->html2pdf->paper('a4', 'portrait');
		
		//Load html view
		$this->html2pdf->html($this->load->view('pdf_potongan', $data, true));
	...
	function printRecords(){
		...
		$data["records"] = $getdata;
		$data["table"] = "potongan";
		$print_view=$this->load->view("p_potongan.php",$data,TRUE);
		if(!file_exists("temp")){
			mkdir("temp");
		}
		$print_file=fopen("temp/potongan.html","w+");
