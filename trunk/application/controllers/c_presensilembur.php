...
class C_presensilembur extends CI_Controller {
	
	function __construct(){
		parent::__construct();		
		$this->load->model('m_presensilembur', '', TRUE);
	}
	...
	function getAll(){
		 ...
		 * Processing Data
		 */
		$result = $this->m_presensilembur->getAll($start, $page, $limit);
	...
	function save(){
		 ...
		 * Processing Data
		 */
		$result = $this->m_presensilembur->save($data);
		echo json_encode($result);
	}
	...
	function delete(){
		 ...
		 * Processing Data
		 */
		$result = $this->m_presensilembur->delete($data);
		echo json_encode($result);
	}
	...
	function export2Excel(){
		...
		foreach($data as $record)
		{
			...
			foreach ($data[0] as $key => $value)
			{
				...
				if($key == strtoupper('presensilembur')){
		...
		$filename='presensilembur.xlsx'; //save our workbook as this file name
	...
	function export2PDF(){
		...
		$this->html2pdf->filename('presensilembur.pdf');
		
		//Set the paper defaults
		$this->html2pdf->paper('a4', 'portrait');
		
		//Load html view
		$this->html2pdf->html($this->load->view('pdf_presensilembur', $data, true));
	...
	function printRecords(){
		...
		$data["records"] = $getdata;
		$data["table"] = "presensilembur";
		$print_view=$this->load->view("p_presensilembur.php",$data,TRUE);
		if(!file_exists("temp")){
			mkdir("temp");
		}
		$print_file=fopen("temp/presensilembur.html","w+");
	...
	function do_upload(){
		...
		else
		{
			...
			$result = $this->m_presensilembur->do_upload($objPHPExcel, $filename);
			echo json_encode($result);
		}
	}
}