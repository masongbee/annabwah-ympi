...
class C_tqcp extends CI_Controller {
	
	function __construct(){
		parent::__construct();		
		$this->load->model('m_tqcp', '', TRUE);
	}
	...
	function getAll(){
		 ...
		 * Processing Data
		 */
		$result = $this->m_tqcp->getAll($start, $page, $limit);
		echo json_encode($result);
	}
	
	function save(){
		/*
		 * Collect Data ==> diambil dari [model.tqcp]
		 */
	...
	function delete(){
		 ...
		 * Processing Data
		 */
		$result = $this->m_tqcp->delete($data);
		echo json_encode($result);
	}
	...
	function export2Excel(){
		...
		$filename='tqcp.xlsx'; //save our workbook as this file name
	...
	function export2PDF(){
		...
		$this->html2pdf->filename('tqcp.pdf');
		
		//Set the paper defaults
		$this->html2pdf->paper('a4', 'portrait');
		
		//Load html view
		$this->html2pdf->html($this->load->view('pdf_tqcp', $data, true));
	...
	function printRecords(){
		...
		$data["records"] = $getdata;
		$data["table"] = "tqcp";
		$print_view=$this->load->view("p_tqcp.php",$data,TRUE);
		if(!file_exists("temp")){
			mkdir("temp");
		}
		$print_file=fopen("temp/tqcp.html","w+");
