...
class C_lembur extends CI_Controller {
	...
	function getAll(){
		 ...
		 * Processing Data
		 */
		$result = $this->m_lembur->getAll($start, $page, $limit);
		echo json_encode($result);
	}
	
	function save(){
		/*
		 * Collect Data ==> diambil dari [model.lembur]
		 */
		 ...
		 * Processing Data
		 */
		$result = $this->m_lembur->save($data);
		echo json_encode($result);
	}
	...
	function delete(){
		 ...
		 * Processing Data
		 */
		$result = $this->m_lembur->delete($data);
		echo json_encode($result);
	}
	...
	function export2Excel(){
		...
		$filename='lembur.xlsx'; //save our workbook as this file name
	...
	function export2PDF(){
		...
		$this->html2pdf->filename('lembur.pdf');
		
		//Set the paper defaults
		$this->html2pdf->paper('a4', 'portrait');
		
		//Load html view
		$this->html2pdf->html($this->load->view('pdf_lembur', $data, true));
	...
	function printRecords(){
		...
		$data["records"] = $getdata;
		$data["table"] = "lembur";
		$print_view=$this->load->view("p_lembur.php",$data,TRUE);
		if(!file_exists("temp")){
			mkdir("temp");
		}
		$print_file=fopen("temp/lembur.html","w+");
	...
	function validtoall_update(){
		 ...
		 * Processing Data
		 */
		$result = $this->m_lembur->validtoall_update($data);
		echo json_encode($result);
	}
}