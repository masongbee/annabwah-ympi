...
class C_ttransport extends CI_Controller {
	
	function __construct(){
		parent::__construct();		
		$this->load->model('m_ttransport', '', TRUE);
	}
	...
	function getAll(){
		 ...
		 * Processing Data
		 */
		$result = $this->m_ttransport->getAll($start, $page, $limit);
		echo json_encode($result);
	}
	
	function save(){
		/*
		 * Collect Data ==> diambil dari [model.ttransport]
		 */
		 ...
		 * Processing Data
		 */
		$result = $this->m_ttransport->save($data);
		echo json_encode($result);
	}
	...
	function delete(){
		 ...
		 * Processing Data
		 */
		$result = $this->m_ttransport->delete($data);
		echo json_encode($result);
	}
	...
	function export2Excel(){
		...
		$filename='ttransport.xlsx'; //save our workbook as this file name
	...
	function export2PDF(){
		...
		$this->html2pdf->filename('ttransport.pdf');
		
		//Set the paper defaults
		$this->html2pdf->paper('a4', 'portrait');
		
		//Load html view
		$this->html2pdf->html($this->load->view('pdf_ttransport', $data, true));
	...
	function printRecords(){
		...
		$data["records"] = $getdata;
		$data["table"] = "ttransport";
		$print_view=$this->load->view("p_ttransport.php",$data,TRUE);
		if(!file_exists("temp")){
			mkdir("temp");
		}
