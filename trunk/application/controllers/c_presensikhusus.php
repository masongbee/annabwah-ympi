...
class C_presensikhusus extends CI_Controller {
	
	function __construct(){
		parent::__construct();		
		$this->load->model('m_presensikhusus', '', TRUE);
	}
	...
	function save(){
		/*
		 * Collect Data ==> diambil dari [model.presensikhusus]
		 */
		 ...
		 * Processing Data
		 */
		$result = $this->m_presensikhusus->save($data);
		echo json_encode($result);
	}
	...
	function delete(){
		 ...
		 * Processing Data
		 */
		$result = $this->m_presensikhusus->delete($data);
		echo json_encode($result);
	}
	...
	function export2Excel(){
		...
		foreach($data as $record)
		{
			...
			foreach ($data[0] as $key => $value)
			{
				...
				if($key == strtoupper('presensikhusus')){
		...
		$filename='presensikhusus.xlsx'; //save our workbook as this file name
	...
	function export2PDF(){
		...
		$this->html2pdf->filename('presensikhusus.pdf');
		
		//Set the paper defaults
		$this->html2pdf->paper('a4', 'portrait');
		
		//Load html view
		$this->html2pdf->html($this->load->view('pdf_presensikhusus', $data, true));
	...
	function printRecords(){
		...
		$data["records"] = $getdata;
		$data["table"] = "presensikhusus";
		$print_view=$this->load->view("p_presensikhusus.php",$data,TRUE);
		if(!file_exists("temp")){
			mkdir("temp");
		}
		$print_file=fopen("temp/presensikhusus.html","w+");
	...
	function do_upload(){
		...
		else
		{
			...
			$result = $this->m_presensikhusus->do_upload($objPHPExcel, $filename);
			echo json_encode($result);
		}
	}
