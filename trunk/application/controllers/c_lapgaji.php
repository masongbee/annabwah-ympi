...
class C_lapgaji extends CI_Controller {
	
	function __construct(){
		parent::__construct();		
		$this->load->model('m_lapgaji', '', TRUE);
	}
	...
	function getAll(){
		...
		$datafilter = json_decode($this->input->post('data',TRUE));
		
		/*
		 * Processing Data
		 */
		$result = $this->m_lapgaji->getAll($start, $page, $limit, $datafilter);
	...
	function export2Excel(){
		...
		$filename='lapgaji.xlsx'; //save our workbook as this file name
	...
	function export2PDF(){
		...
		$this->html2pdf->filename('lapgaji.pdf');
		
		//Set the paper defaults
		$this->html2pdf->paper('a4', 'portrait');
		
		//Load html view
		$this->html2pdf->html($this->load->view('pdf_lapgaji', $data, true));
	...
	function printRecords(){
		...
		$data["records"] = $getdata;
		$data["table"] = "lapgaji";
		$print_view=$this->load->view("p_lapgaji.php",$data,TRUE);
		if(!file_exists("temp")){
			mkdir("temp");
		}
		$print_file=fopen("temp/lapgaji.html","w+");
