 ...
 *
 */
class M_potonganlain2 extends CI_Model{
	...
	function getAll($start, $page, $limit){
		//$query  = $this->db->limit($limit, $start)->order_by('NOURUT', 'ASC')->get('potonganlain2')->result();
		...
		$total  = $this->db->get('potonganlain2')->num_rows();
	...
	function save($data){
		...
		if($this->db->get_where('potonganlain2', $pkey)->num_rows() > 0){
			/*
			 * Data Exist
			 */
			...
			$arrdatau = array(
				...
				'RPPOTONGAN'=>(trim($data->RPPOTONGAN) == '' ? 0 : $data->RPPOTONGAN),
				'USERNAME'=>$data->USERNAME
			);
			
			$this->db->where($pkey)->update('potonganlain2', $arrdatau);
			$last   = $data;
			 ...
			 * Process Insert
			 */
			$nourut_last = $this->db->select('COUNT(*) AS total')->where('BULAN', date('Ym', strtotime($data->BULAN)))->get('potonganlain2')->row();
			...
			$this->db->insert('potonganlain2', $arrdatac);
			$last   = $this->db->where($pkey)->get('potonganlain2')->row();
	...
	function delete($data){
		...
		$this->db->where($pkey)->delete('potonganlain2');
		
		$total  = $this->db->get('potonganlain2')->num_rows();
		$last = $this->db->get('potonganlain2')->result();
				...
				for ($row = 1; $row <= $highestRow; ++ $row) {
					if($row>1){
						for ($col = 0; $col < $highestColumnIndex; ++ $col) {
							...
							$kodepotongan = (trim($worksheet->getCellByColumnAndRow(2, $row)->getValue()) == ''? NULL : trim($worksheet->getCellByColumnAndRow(2, $row)->getValue()));
							...
							$rppotongan = ($worksheet->getCellByColumnAndRow(8, $row)->getValue() == ''? 0 : $worksheet->getCellByColumnAndRow(8, $row)->getValue());
						}
						...
						$data = array(
							...
							'RPPOTONGAN' => $rppotongan
						);
						if($this->db->get_where('potonganlain2', array('BULAN'=>$bulan,'NOURUT'=>$nourut))->num_rows() == 0){
							$this->db->insert('potonganlain2', $data);
						}else{
							$skeepdata++;
						}
