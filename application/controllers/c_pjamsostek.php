...
class C_pjamsostek extends CI_Controller {
	
	function __construct(){
		parent::__construct();		
		$this->load->model('m_pjamsostek', '', TRUE);
	}
	...
	function getAll(){
		 ...
		 * Processing Data
		 */
		$result = $this->m_pjamsostek->getAll($start, $page, $limit);
		echo json_encode($result);
	}
	
	function save(){
		/*
		 * Collect Data ==> diambil dari [model.pjamsostek]
		 */
		 ...
		 * Processing Data
		 */
		$result = $this->m_pjamsostek->save($data);
		echo json_encode($result);
	}
	...
	function delete(){
		 ...
		 * Processing Data
		 */
		$result = $this->m_pjamsostek->delete($data);
		echo json_encode($result);
	}
	...
	function export2Excel(){
		...
		$filename='pjamsostek.xlsx'; //save our workbook as this file name
	...
	function export2PDF(){
		...
		$this->html2pdf->filename('pjamsostek.pdf');
		
		//Set the paper defaults
		$this->html2pdf->paper('a4', 'portrait');
		
		//Load html view
		$this->html2pdf->html($this->load->view('pdf_pjamsostek', $data, true));
	...
	function printRecords(){
		...
		$data["records"] = $getdata;
		$data["table"] = "pjamsostek";
		$print_view=$this->load->view("p_pjamsostek.php",$data,TRUE);
		if(!file_exists("temp")){
			mkdir("temp");
		}
		$print_file=fopen("temp/pjamsostek.html","w+");
