...
class C_jnsseleksi extends CI_Controller {
	
	function __construct(){
		parent::__construct();		
		$this->load->model('m_jnsseleksi', '', TRUE);
	}
	...
	function getAll(){
		 ...
		 * Processing Data
		 */
		$result = $this->m_jnsseleksi->getAll($start, $page, $limit);
		echo json_encode($result);
	}
	
	function save(){
		/*
		 * Collect Data ==> diambil dari [model.jnsseleksi]
		 */
		 ...
		 * Processing Data
		 */
		$result = $this->m_jnsseleksi->save($data);
		echo json_encode($result);
	}
	...
	function delete(){
		 ...
		 * Processing Data
		 */
		$result = $this->m_jnsseleksi->delete($data);
		echo json_encode($result);
	}
	...
	function export2Excel(){
		...
		$filename='jnsseleksi.xlsx'; //save our workbook as this file name
	...
	function export2PDF(){
		...
		$this->html2pdf->filename('jnsseleksi.pdf');
		
		//Set the paper defaults
		$this->html2pdf->paper('a4', 'portrait');
		
		//Load html view
		$this->html2pdf->html($this->load->view('pdf_jnsseleksi', $data, true));
	...
	function printRecords(){
		...
		$data["records"] = $getdata;
		$data["table"] = "jnsseleksi";
		$print_view=$this->load->view("p_jnsseleksi.php",$data,TRUE);
		if(!file_exists("temp")){
			mkdir("temp");
		}
		$print_file=fopen("temp/jnsseleksi.html","w+");
	...
	function getNamaAlternatif(){
		 ...
		 * Processing Data
		 */
		$result = $this->m_jnsseleksi->getNamaAlternatif();
		echo $result;
	}
}