...
class C_keluarga extends CI_Controller {
	...
	function __construct(){
		parent::__construct();		
		$this->load->model('m_keluarga', '', TRUE);
	}
	...
	function getAll(){
		 ...
		 * Processing Data
		 */
		$result = $this->m_keluarga->getAll($nik, $start, $page, $limit);
		echo json_encode($result);
	}
	
	function save(){
		/*
		 * Collect Data ==> diambil dari [model.keluarga]
		 */
		 ...
		 * Processing Data
		 */
		$result = $this->m_keluarga->save($data);
		echo json_encode($result);
	}
	...
	function delete(){
		 ...
		 * Processing Data
		 */
		$result = $this->m_keluarga->delete($data);
		echo json_encode($result);
	}
	...
	function export2Excel(){
		...
		$filename='keluarga.xlsx'; //save our workbook as this file name
	...
	function export2PDF(){
		...
		$this->html2pdf->filename('keluarga.pdf');
		
		//Set the paper defaults
		$this->html2pdf->paper('a4', 'portrait');
		
		//Load html view
		$this->html2pdf->html($this->load->view('pdf_keluarga', $data, true));
	...
	function printRecords(){
		...
		$data["records"] = $getdata;
		$data["table"] = "keluarga";
		$print_view=$this->load->view("p_keluarga.php",$data,TRUE);
		if(!file_exists("temp")){
			mkdir("temp");
		}
		$print_file=fopen("temp/keluarga.html","w+");
