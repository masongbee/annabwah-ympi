...
class C_tkacamata extends CI_Controller {
	
	function __construct(){
		parent::__construct();		
		$this->load->model('m_tkacamata', '', TRUE);
	}
	...
	function getAll(){
		 ...
		 * Processing Data
		 */
		$result = $this->m_tkacamata->getAll($start, $page, $limit);
	...
	function save(){
		 ...
		 * Processing Data
		 */
		$result = $this->m_tkacamata->save($data);
		echo json_encode($result);
	}
	...
	function delete(){
		 ...
		 * Processing Data
		 */
		$result = $this->m_tkacamata->delete($data);
		echo json_encode($result);
	}
	...
	function export2Excel(){
		...
		$filename='tkacamata.xlsx'; //save our workbook as this file name
	...
	function export2PDF(){
		...
		$this->html2pdf->filename('tkacamata.pdf');
		
		//Set the paper defaults
		$this->html2pdf->paper('a4', 'portrait');
		
		//Load html view
		$this->html2pdf->html($this->load->view('pdf_tkacamata', $data, true));
	...
	function printRecords(){
		...
		$data["records"] = $getdata;
		$data["table"] = "tkacamata";
		$print_view=$this->load->view("p_tkacamata.php",$data,TRUE);
		if(!file_exists("temp")){
			mkdir("temp");
		}
		$print_file=fopen("temp/tkacamata.html","w+");
