...
class C_tbhs extends CI_Controller {
	
	function __construct(){
		parent::__construct();		
		$this->load->model('m_tbhs', '', TRUE);
	}
	...
	function getAll(){
		 ...
		 * Processing Data
		 */
		$result = $this->m_tbhs->getAll($start, $page, $limit);
		echo json_encode($result);
	}
	
	function save(){
		/*
		 * Collect Data ==> diambil dari [model.tbhs]
		 */
	...
	function delete(){
		 ...
		 * Processing Data
		 */
		$result = $this->m_tbhs->delete($data);
		echo json_encode($result);
	}
	...
	function export2Excel(){
		...
		$filename='tbhs.xlsx'; //save our workbook as this file name
	...
	function export2PDF(){
		...
		$this->html2pdf->filename('tbhs.pdf');
		
		//Set the paper defaults
		$this->html2pdf->paper('a4', 'portrait');
		
		//Load html view
		$this->html2pdf->html($this->load->view('pdf_tbhs', $data, true));
	...
	function printRecords(){
		...
		$data["records"] = $getdata;
		$data["table"] = "tbhs";
		$print_view=$this->load->view("p_tbhs.php",$data,TRUE);
		if(!file_exists("temp")){
			mkdir("temp");
		}
		$print_file=fopen("temp/tbhs.html","w+");
	...
	function validtoall_update(){
		 ...
		 * Processing Data
		 */
		$result = $this->m_tbhs->validtoall_update($data);
		echo json_encode($result);
	}
}