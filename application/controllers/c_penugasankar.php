...
class C_penugasankar extends CI_Controller {
	
	function __construct(){
		parent::__construct();		
		$this->load->model('m_penugasankar', '', TRUE);
	}
	...
	function getAll(){
		 ...
		 * Processing Data
		 */
		$result = $this->m_penugasankar->getAll($start, $page, $limit);
		echo json_encode($result);
	}
	
	function save(){
		/*
		 * Collect Data ==> diambil dari [model.penugasankar]
		 */
		 ...
		 * Processing Data
		 */
		$result = $this->m_penugasankar->save($data);
		echo json_encode($result);
	}
	...
	function delete(){
		 ...
		 * Processing Data
		 */
		$result = $this->m_penugasankar->delete($data);
		echo json_encode($result);
	}
	...
	function do_upload(){
		...
		else
		{
			...
			$result = $this->m_penugasankar->do_upload($objPHPExcel, $filename);
			echo json_encode($result);
		}
	}

	function lappenugasankar(){
		/*
		 * Collect Data
		 */
		 ...
		 * Processing Data
		 */
		$result = $this->m_penugasankar->lappenugasankar($bulan, $tglmulai, $tglsampai);
		echo json_encode($result);
	}

	function lappenugasankarExport2Excel(){
		$bulan     =   ($this->input->post('bulan', TRUE) ? $this->input->post('bulan', TRUE) : '');
		$tglmulai  = ($this->input->post('tglmulai', TRUE) ? $this->input->post('tglmulai', TRUE) : '');
		$tglsampai = ($this->input->post('tglsampai', TRUE) ? $this->input->post('tglsampai', TRUE) : '');
		...
		$records = $this->m_penugasankar->lappenugasankar($bulan, $tglmulai, $tglsampai);
	...
	function lappenugasankarExport2PDF(){
		$bulan     =   ($this->input->post('bulan', TRUE) ? $this->input->post('bulan', TRUE) : '');
		$tglmulai  = ($this->input->post('tglmulai', TRUE) ? $this->input->post('tglmulai', TRUE) : '');
		$tglsampai = ($this->input->post('tglsampai', TRUE) ? $this->input->post('tglsampai', TRUE) : '');
