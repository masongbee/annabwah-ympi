...
class C_cicilan extends CI_Controller {
	
	function __construct(){
		parent::__construct();		
		$this->load->model('m_cicilan', '', TRUE);
	}
	...
	function getAll(){
		 ...
		 * Processing Data
		 */
		$result = $this->m_cicilan->getAll($start, $page, $limit);
		echo json_encode($result);
	}
	
	function save(){
		/*
		 * Collect Data ==> diambil dari [model.cicilan]
		 */
		 ...
		 * Processing Data
		 */
		$result = $this->m_cicilan->save($data);
		echo json_encode($result);
	}
	...
	function delete(){
		 ...
		 * Processing Data
		 */
		$result = $this->m_cicilan->delete($data);
		echo json_encode($result);
	}
	...
	function export2Excel(){
		...
		$filename='cicilan.xlsx'; //save our workbook as this file name
	...
	function export2PDF(){
		...
		$this->html2pdf->filename('cicilan.pdf');
		
		//Set the paper defaults
		$this->html2pdf->paper('a4', 'portrait');
		
		//Load html view
		$this->html2pdf->html($this->load->view('pdf_cicilan', $data, true));
	...
	function printRecords(){
		...
		$data["records"] = $getdata;
		$data["table"] = "cicilan";
		$print_view=$this->load->view("p_cicilan.php",$data,TRUE);
		if(!file_exists("temp")){
			mkdir("temp");
		}
		$print_file=fopen("temp/cicilan.html","w+");
