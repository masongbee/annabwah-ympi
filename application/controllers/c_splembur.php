...
class C_splembur extends CI_Controller {
	...
	function save(){
		/*
		 * Collect Data ==> diambil dari [model.splembur]
		 */
		 ...
		 * Processing Data
		 */
		$result = $this->m_splembur->save($data);
		echo json_encode($result);
	}
	...
	function delete(){
		 ...
		 * Processing Data
		 */
		$result = $this->m_splembur->delete($data);
		echo json_encode($result);
	}
	...
	function export2Excel(){
		...
		$filename='splembur.xlsx'; //save our workbook as this file name
	...
	function export2PDF(){
		...
		$this->html2pdf->filename('splembur.pdf');
		
		//Set the paper defaults
		$this->html2pdf->paper('a4', 'portrait');
		
		//Load html view
		$this->html2pdf->html($this->load->view('pdf_splembur', $data, true));
	...
	function printRecords(){
		...
		$data["records"] = $getdata;
		$data["table"] = "splembur";
		$print_view=$this->load->view("p_splembur.php",$data,TRUE);
		if(!file_exists("temp")){
			mkdir("temp");
		}
		$print_file=fopen("temp/splembur.html","w+");
