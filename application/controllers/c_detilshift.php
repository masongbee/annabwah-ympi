...
class C_detilshift extends CI_Controller {
	
	function __construct(){
		parent::__construct();		
		$this->load->model('m_detilshift', '', TRUE);
	}
	...
	function getAll(){
		 ...
		 * Processing Data
		 */
		$result = $this->m_detilshift->getAll($namashift,$start, $page, $limit);
		echo json_encode($result);
	}
	
	function save(){
		/*
		 * Collect Data ==> diambil dari [model.detilshift]
		 */
		 ...
		 * Processing Data
		 */
		$result = $this->m_detilshift->save($data);
		echo json_encode($result);
	}
	...
	function delete(){
		 ...
		 * Processing Data
		 */
		$result = $this->m_detilshift->delete($data);
		echo json_encode($result);
	}
	...
	function export2Excel(){
		...
		$filename='detilshift.xlsx'; //save our workbook as this file name
	...
	function export2PDF(){
		...
		$this->html2pdf->filename('detilshift.pdf');
		
		//Set the paper defaults
		$this->html2pdf->paper('a4', 'portrait');
		
		//Load html view
		$this->html2pdf->html($this->load->view('pdf_detilshift', $data, true));
	...
	function printRecords(){
		...
		$data["records"] = $getdata;
		$data["table"] = "detilshift";
		$print_view=$this->load->view("p_detilshift.php",$data,TRUE);
		if(!file_exists("temp")){
			mkdir("temp");
		}
		$print_file=fopen("temp/detilshift.html","w+");
