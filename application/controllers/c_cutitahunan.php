...
class C_cutitahunan extends CI_Controller {
	
	function __construct(){
		parent::__construct();		
		$this->load->model('m_cutitahunan', '', TRUE);
	}
	...
	function getAll(){
		 ...
		 * Processing Data
		 */
		$result = $this->m_cutitahunan->getAll($start, $page, $limit, $filter);
		echo json_encode($result);
	}
	
	function save(){
		/*
		 * Collect Data ==> diambil dari [model.cutitahunan]
		 */
		 ...
		 * Processing Data
		 */
		$result = $this->m_cutitahunan->save($data);
		echo json_encode($result);
	}
	...
	function delete(){
		 ...
		 * Processing Data
		 */
		$result = $this->m_cutitahunan->delete($data);
		echo json_encode($result);
	}
	...
	function export2Excel(){
		...
		$filename='cutitahunan.xlsx'; //save our workbook as this file name
	...
	function export2PDF(){
		...
		$this->html2pdf->filename('cutitahunan.pdf');
		
		//Set the paper defaults
		$this->html2pdf->paper('a4', 'portrait');
		
		//Load html view
		$this->html2pdf->html($this->load->view('pdf_cutitahunan', $data, true));
	...
	function printRecords(){
		...
		$data["records"] = $getdata;
		$data["table"] = "cutitahunan";
		$print_view=$this->load->view("p_cutitahunan.php",$data,TRUE);
		if(!file_exists("temp")){
			mkdir("temp");
		}
