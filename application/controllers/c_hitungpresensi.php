...
class C_hitungpresensi extends CI_Controller {
	...
	function get_periodegaji(){
		$result = $this->m_hitungpresensi->get_periodegaji();
		echo json_encode($result);
	}
	
	function hitungpresensi_all(){
		$bulan = $this->input->post('bulan', TRUE);
		
		$result = $this->m_hitungpresensi->hitungpresensi_all($bulan);
		return $result;
	}
	...
	function save(){
		/*
		 * Collect Data ==> diambil dari [model.hitungpresensi]
		 */
	...
	function export2Excel(){
		...
		foreach($data as $record)
		{
			...
			foreach ($data[0] as $key => $value)
			{
				...
				if($key == strtoupper('hitungpresensi')){
		...
		$filename='hitungpresensi.xlsx'; //save our workbook as this file name
	...
	function export2PDF(){
		...
		$this->html2pdf->filename('hitungpresensi.pdf');
	...
	function printRecords(){
		...
		$data["records"] = $getdata;
		$data["table"] = "hitungpresensi";
		$print_view=$this->load->view("p_hitungpresensi.php",$data,TRUE);
		if(!file_exists("temp")){
			mkdir("temp");
		}
		$print_file=fopen("temp/hitungpresensi.html","w+");
