...
class C_td_kelompok extends CI_Controller {
	
	function __construct(){
		parent::__construct();		
		$this->load->model('m_td_kelompok', '', TRUE);
	}
	...
	function getAll(){
		 ...
		 * Processing Data
		 */
		$result = $this->m_td_kelompok->getAll($start, $page, $limit);
		echo json_encode($result);
	}
	
	function save(){
		/*
		 * Collect Data ==> diambil dari [model.td_kelompok]
		 */
		 ...
		 * Processing Data
		 */
		$result = $this->m_td_kelompok->save($data);
		echo json_encode($result);
	}
	...
	function delete(){
		 ...
		 * Processing Data
		 */
		$result = $this->m_td_kelompok->delete($data);
		echo json_encode($result);
	}
	...
	function export2Excel(){
		...
		$filename='td_kelompok.xlsx'; //save our workbook as this file name
	...
	function export2PDF(){
		...
		$this->html2pdf->filename('td_kelompok.pdf');
		
		//Set the paper defaults
		$this->html2pdf->paper('a4', 'portrait');
		
		//Load html view
		$this->html2pdf->html($this->load->view('pdf_td_kelompok', $data, true));
	...
	function printRecords(){
		...
		$data["records"] = $getdata;
		$data["table"] = "td_kelompok";
		$print_view=$this->load->view("p_td_kelompok.php",$data,TRUE);
		if(!file_exists("temp")){
			mkdir("temp");
		}
		$print_file=fopen("temp/td_kelompok.html","w+");
