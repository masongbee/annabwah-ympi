...
class C_bonus extends CI_Controller {
	
	function __construct(){
		parent::__construct();		
		$this->load->model('m_bonus', '', TRUE);
	}
	...
	function getAll(){
		 ...
		 * Processing Data
		 */
		$result = $this->m_bonus->getAll($start, $page, $limit);
		echo json_encode($result);
	}
	
	function save(){
		/*
		 * Collect Data ==> diambil dari [model.bonus]
		 */
	...
	function delete(){
		 ...
		 * Processing Data
		 */
		$result = $this->m_bonus->delete($data);
		echo json_encode($result);
	}
	...
	function export2Excel(){
		...
		$filename='bonus.xlsx'; //save our workbook as this file name
	...
	function export2PDF(){
		...
		$this->html2pdf->filename('bonus.pdf');
		
		//Set the paper defaults
		$this->html2pdf->paper('a4', 'portrait');
		
		//Load html view
		$this->html2pdf->html($this->load->view('pdf_bonus', $data, true));
	...
	function printRecords(){
		...
		$data["records"] = $getdata;
		$data["table"] = "bonus";
		$print_view=$this->load->view("p_bonus.php",$data,TRUE);
		if(!file_exists("temp")){
			mkdir("temp");
		}
		$print_file=fopen("temp/bonus.html","w+");
