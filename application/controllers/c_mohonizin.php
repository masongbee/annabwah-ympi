...
class C_mohonizin extends CI_Controller {
	...
	function getAll(){
		 ...
		 * Processing Data
		 */
		$result = $this->m_mohonizin->getAll($start, $page, $limit);
		echo json_encode($result);
	}
	
	function save(){
		/*
		 * Collect Data ==> diambil dari [model.mohonizin]
		 */
		 ...
		 * Processing Data
		 */
		$result = $this->m_mohonizin->save($data);
		echo json_encode($result);
	}
	...
	function delete(){
		 ...
		 * Processing Data
		 */
		$result = $this->m_mohonizin->delete($data);
		echo json_encode($result);
	}
	...
	function export2Excel(){
		...
		$filename='mohonizin.xlsx'; //save our workbook as this file name
	...
	function export2PDF(){
		...
		$this->html2pdf->filename('mohonizin.pdf');
		
		//Set the paper defaults
		$this->html2pdf->paper('a4', 'portrait');
		
		//Load html view
		$this->html2pdf->html($this->load->view('pdf_mohonizin', $data, true));
	...
	function printRecords(){
		...
		$data["records"] = $getdata;
		$data["table"] = "PERMOHONANIJIN";
		$print_view=$this->load->view("p_mohonizin.php",$data,TRUE);
		if(!file_exists("temp")){
			mkdir("temp");
		}
		$print_file=fopen("temp/mohonizin.html","w+");
