...
class C_kalenderlibur extends CI_Controller {
	
	function __construct(){
		parent::__construct();		
		$this->load->model('m_kalenderlibur', '', TRUE);
	}
	...
	function getAll(){
		 ...
		 * Processing Data
		 */
		$result = $this->m_kalenderlibur->getAll($tglmulai, $tglsampai, $start, $page, $limit);
		echo json_encode($result);
	}
	
	function save(){
		/*
		 * Collect Data ==> diambil dari [model.kalenderlibur]
		 */
		 ...
		 * Processing Data
		 */
		$result = $this->m_kalenderlibur->save($data);
		echo json_encode($result);
	}
	...
	function delete(){
		 ...
		 * Processing Data
		 */
		$result = $this->m_kalenderlibur->delete($data);
		echo json_encode($result);
	}
	...
	function export2Excel(){
		...
		foreach($data as $record)
		{
			...
			foreach ($data[0] as $key => $value)
			{
				...
				if($key == strtoupper('kalenderlibur')){
		...
		$filename='kalenderlibur.xlsx'; //save our workbook as this file name
	...
	function export2PDF(){
		...
		$this->html2pdf->filename('kalenderlibur.pdf');
		
		//Set the paper defaults
		$this->html2pdf->paper('a4', 'portrait');
		
		//Load html view
		$this->html2pdf->html($this->load->view('pdf_kalenderlibur', $data, true));
	...
	function printRecords(){
		...
		$data["records"] = $getdata;
		$data["table"] = "kalenderlibur";
		$print_view=$this->load->view("p_kalenderlibur.php",$data,TRUE);
		if(!file_exists("temp")){
			mkdir("temp");
		}
		$print_file=fopen("temp/kalenderlibur.html","w+");
