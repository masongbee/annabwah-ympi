...
class C_tambahanlain2 extends CI_Controller {
	
	function __construct(){
		parent::__construct();		
		$this->load->model('m_tambahanlain2', '', TRUE);
	}
	...
	function getAll(){
		 ...
		 * Processing Data
		 */
		$result = $this->m_tambahanlain2->getAll($start, $page, $limit);
		echo json_encode($result);
	}
	
	function save(){
		/*
		 * Collect Data ==> diambil dari [model.tambahanlain2]
		 */
		 ...
		 * Processing Data
		 */
		$result = $this->m_tambahanlain2->save($data);
		echo json_encode($result);
	}
	...
	function delete(){
		 ...
		 * Processing Data
		 */
		$result = $this->m_tambahanlain2->delete($data);
		echo json_encode($result);
	}
	...
	function export2Excel(){
		...
		foreach($data as $record)
		{
			...
			foreach ($data[0] as $key => $value)
			{
				...
				if($key == strtoupper('tambahanlain2')){
		...
		$filename='tambahanlain2.xlsx'; //save our workbook as this file name
	...
	function export2PDF(){
		...
		$this->html2pdf->filename('tambahanlain2.pdf');
		
		//Set the paper defaults
		$this->html2pdf->paper('a4', 'portrait');
		
		//Load html view
		$this->html2pdf->html($this->load->view('pdf_tambahanlain2', $data, true));
	...
	function printRecords(){
		...
		$data["records"] = $getdata;
		$data["table"] = "tambahanlain2";
		$print_view=$this->load->view("p_tambahanlain2.php",$data,TRUE);
		if(!file_exists("temp")){
			mkdir("temp");
		}
		$print_file=fopen("temp/tambahanlain2.html","w+");
	...
	function do_upload(){
		...
		else
		{
			...
			$result = $this->m_tambahanlain2->do_upload($objPHPExcel, $filename);
			echo json_encode($result);
		}
	}
}