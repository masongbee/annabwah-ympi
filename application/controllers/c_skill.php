...
class C_skill extends CI_Controller {
	
	function __construct(){
		parent::__construct();		
		$this->load->model('m_skill', '', TRUE);
	}
	...
	function getAll(){
		 ...
		 * Processing Data
		 */
		$result = $this->m_skill->getAll($nik, $start, $page, $limit);
	...
	function delete(){
		 ...
		 * Processing Data
		 */
		$result = $this->m_skill->delete($data);
		echo json_encode($result);
	}
	...
	function export2Excel(){
		...
		$filename='skill.xlsx'; //save our workbook as this file name
	...
	function export2PDF(){
		...
		$this->html2pdf->filename('skill.pdf');
		
		//Set the paper defaults
		$this->html2pdf->paper('a4', 'portrait');
		
		//Load html view
		$this->html2pdf->html($this->load->view('pdf_skill', $data, true));
	...
	function printRecords(){
		...
		$data["records"] = $getdata;
		$data["table"] = "skill";
		$print_view=$this->load->view("p_skill.php",$data,TRUE);
		if(!file_exists("temp")){
			mkdir("temp");
		}
		$print_file=fopen("temp/skill.html","w+");
