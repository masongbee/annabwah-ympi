...
class C_periodegaji extends CI_Controller {
	
	function __construct(){
		parent::__construct();		
		$this->load->model('m_periodegaji', '', TRUE);
	}
	...
	function getAll(){
		 ...
		 * Processing Data
		 */
		$result = $this->m_periodegaji->getAll($start, $page, $limit);
		echo json_encode($result);
	}
	
	function save(){
		/*
		 * Collect Data ==> diambil dari [model.periodegaji]
		 */
		 ...
		 * Processing Data
		 */
		$result = $this->m_periodegaji->save($data);
		echo json_encode($result);
	}
	...
	function delete(){
		 ...
		 * Processing Data
		 */
		$result = $this->m_periodegaji->delete($data);
		echo json_encode($result);
	}
	...
	function export2Excel(){
		...
		$filename='periodegaji.xlsx'; //save our workbook as this file name
	...
	function export2PDF(){
		...
		$this->html2pdf->filename('periodegaji.pdf');
		
		//Set the paper defaults
		$this->html2pdf->paper('a4', 'portrait');
		
		//Load html view
		$this->html2pdf->html($this->load->view('pdf_periodegaji', $data, true));
	...
	function printRecords(){
		...
		$data["records"] = $getdata;
		$data["table"] = "periodegaji";
		$print_view=$this->load->view("p_periodegaji.php",$data,TRUE);
		if(!file_exists("temp")){
			mkdir("temp");
		}
		$print_file=fopen("temp/periodegaji.html","w+");
