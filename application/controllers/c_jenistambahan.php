...
class C_jenistambahan extends CI_Controller {
	
	function __construct(){
		parent::__construct();		
		$this->load->model('m_jenistambahan', '', TRUE);
	}
	...
	function getAll(){
		 ...
		 * Processing Data
		 */
		$result = $this->m_jenistambahan->getAll($start, $page, $limit);
		echo json_encode($result);
	}
	
	function save(){
		/*
		 * Collect Data ==> diambil dari [model.jenistambahan]
		 */
		 ...
		 * Processing Data
		 */
		$result = $this->m_jenistambahan->save($data);
		echo json_encode($result);
	}
	...
	function delete(){
		 ...
		 * Processing Data
		 */
		$result = $this->m_jenistambahan->delete($data);
		echo json_encode($result);
	}
	...
	function export2Excel(){
		...
		foreach($data as $record)
		{
			...
			foreach ($data[0] as $key => $value)
			{
				...
				if($key == strtoupper('jenistambahan')){
		...
		$filename='jenistambahan.xlsx'; //save our workbook as this file name
	...
	function export2PDF(){
		...
		$this->html2pdf->filename('jenistambahan.pdf');
		
		//Set the paper defaults
		$this->html2pdf->paper('a4', 'portrait');
		
		//Load html view
		$this->html2pdf->html($this->load->view('pdf_jenistambahan', $data, true));
	...
	function printRecords(){
		...
		$data["records"] = $getdata;
		$data["table"] = "jenistambahan";
		$print_view=$this->load->view("p_jenistambahan.php",$data,TRUE);
		if(!file_exists("temp")){
			mkdir("temp");
		}
