...
class C_td_evefektivitas extends CI_Controller {
	
	function __construct(){
		parent::__construct();		
		$this->load->model('m_td_evefektivitas', '', TRUE);
	}
	...
	function getAll(){
		 ...
		 * Processing Data
		 */
		$result = $this->m_td_evefektivitas->getAll($start, $page, $limit);
		echo json_encode($result);
	}
	
	function save(){
		/*
		 * Collect Data ==> diambil dari [model.td_evefektivitas]
		 */
		 ...
		 * Processing Data
		 */
		$result = $this->m_td_evefektivitas->save($data);
		echo json_encode($result);
	}
	...
	function delete(){
		 ...
		 * Processing Data
		 */
		$result = $this->m_td_evefektivitas->delete($data);
		echo json_encode($result);
	}
	...
	function export2Excel(){
		...
		foreach($data as $record)
		{
			...
			foreach ($data[0] as $key => $value)
			{
				...
				if($key == strtoupper('td_evefektivitas')){
		...
		$filename='td_evefektivitas.xlsx'; //save our workbook as this file name
	...
	function export2PDF(){
		...
		$this->html2pdf->filename('td_evefektivitas.pdf');
		
		//Set the paper defaults
		$this->html2pdf->paper('a4', 'portrait');
		
		//Load html view
		$this->html2pdf->html($this->load->view('pdf_td_evefektivitas', $data, true));
	...
	function printRecords(){
		...
		$data["records"] = $getdata;
		$data["table"] = "td_evefektivitas";
		$print_view=$this->load->view("p_td_evefektivitas.php",$data,TRUE);
		if(!file_exists("temp")){
			mkdir("temp");
		}
		$print_file=fopen("temp/td_evefektivitas.html","w+");
